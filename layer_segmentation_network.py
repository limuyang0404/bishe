...
#     return
def GaussianInitialize(layer, n):
    # layer_size = list(layer.weight.size())
    layer_size = layer.weight.size()
    # layer_point = 1
    # for i in range(len(layer_size)):
    #     layer_point = layer_point * layer_size[i]
    initialize = np.random.normal(0, (2/n)**0.5, layer_point)
    #mean 0 variance 2/layer_point
    initialize = initialize.reshape(layer_size)
    layer.weight = nn.Parameter(torch.Tensor(initialize))
    return
...
class U_Net(nn.Module):
    def __init__(self,n_classes=2):
        ...
        self.deconv_1 = nn.ConvTranspose3d(in_channels=256, out_channels=128, kernel_size=2, stride=3, padding=3)
        self.right_conv_1 = Conv3d_block(in_channels=256, out_channels=128)
        self.deconv_2 = nn.ConvTranspose3d(in_channels=128, out_channels=64, kernel_size=3, stride=3, padding=7)
    ...
    def ParameterInitialize(self):
        GaussianInitialize(self.left_conv_1.conv_ReLU[0], 32)
        GaussianInitialize(self.left_conv_1.conv_ReLU[3], 32)
        GaussianInitialize(self.left_conv_2.conv_ReLU[0], 64)
        GaussianInitialize(self.left_conv_2.conv_ReLU[3], 64)
        GaussianInitialize(self.left_conv_3.conv_ReLU[0], 128)
        GaussianInitialize(self.left_conv_3.conv_ReLU[3], 128)
        GaussianInitialize(self.left_conv_4.conv_ReLU[0], 256)
        GaussianInitialize(self.left_conv_4.conv_ReLU[3], 256)
        GaussianInitialize(self.right_conv_1.conv_ReLU[0], 128)
        GaussianInitialize(self.right_conv_1.conv_ReLU[3], 128)
        GaussianInitialize(self.right_conv_2.conv_ReLU[0], 64)
        GaussianInitialize(self.right_conv_2.conv_ReLU[3], 64)
        GaussianInitialize(self.right_conv_3.conv_ReLU[0], 32)
        GaussianInitialize(self.right_conv_3.conv_ReLU[3], 32)
        GaussianInitialize(self.right_conv_4.conv_ReLU[0], self.ncls)
        GaussianInitialize(self.right_conv_4.conv_ReLU[3], self.ncls)
...
if __name__ == "__main__":
    x1 = torch.rand(size=(32, 1, 56, 56, 56))
    net = U_Net(n_classes=2)
    print('net dir:\n', dir(net))
    print('The net:', net)
    print('*'*60)
