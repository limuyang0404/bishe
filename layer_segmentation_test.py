...
class LayerModel(object):
    ...
    def structured_grid(self):
        _, _, _, self.structured_grid_r, self.structured_grid_cls, self.structured_grid_amp, _ = model_structure_grid1(self.saved_model[0, :], self.saved_model[1, :], self.saved_model[2, :], self.saved_model[3, :], self.saved_model[5, :], self.saved_model[4, :], origin_data_size=self.layer_size, structure_grid_size=140)
...
if __name__ == '__main__':
    ...
    for z in range(20):
        ...
        # model.fold_and_fault_generate(random_seed=np.random.randint(20000))
        # model.structured_grid()
        np.savetxt(r'layer_segmentation_test_model_2/' + str(z+1) + '.txt', np.vstack([model.output_model_amp_norm.flatten(), model.output_model_cls.flatten()]))
        print('The ' + str(z+1) + 'th model have generated!')
